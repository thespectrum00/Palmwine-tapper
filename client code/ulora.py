...
class LoRa(object):
    def __init__(self, spi_channel, interrupt, this_address, cs_pin, reset_pin=None, freq=868.0, tx_power=14,
        ...
        # Now register interrupt pin (moved after SPI/CS init for safety)
        # Use the provided pull mode (helps prevent floating)
        try:
            gpio_interrupt = Pin(self._interrupt, Pin.IN, interrupt_pull)
        except TypeError:
            # Some MicroPython builds may not accept pull arg - fallback
            gpio_interrupt = Pin(self._interrupt, Pin.IN)
        gpio_interrupt.irq(trigger=Pin.IRQ_RISING, handler=self._handle_interrupt)

        # set mode to sleep long range then check
    ...
    # --- rest of methods unchanged (send, set_mode_rx, etc) ---
    ...
    def set_mode_tx(self):
        if self._mode != MODE_TX:
            ...
            self._spi_write(REG_40_DIO_MAPPING1, 0x40)
            self._mode = MODE_TX
    ...
    def set_mode_cad(self):
        if self._mode != MODE_CAD:
            ...
            self._spi_write(REG_40_DIO_MAPPING1, 0x80)
            self._mode = MODE_CAD
    ...
    def _handle_interrupt(self, channel):
        # Debug: if spi not ready, ignore
        if not hasattr(self, 'spi'):
            print("IRQ fired before SPI init - ignoring")
            return
        ...
        if self._mode == MODE_RXCONTINUOUS and (irq_flags & RX_DONE):
            ...
            if packet_len >= 4:
                ...
                # debug
                print("hdr to/from/flags:", header_to, header_from, header_flags)
